...
class Sets
{
    static void Show(string name, GearParams p)
    {
        ...
        Console.WriteLine(string.Format("  mn={0} z={1} a={2} b(螺旋)={3} x={4} 齿宽={5} 内孔={6} 键槽={7}",
            p.Mn, p.Z, p.AlfN, p.Beta, p.X, p.Bw, p.Bore, p.Keyway ? "是" : "否"));
        ...
        if (ky != null)
            Console.WriteLine(string.Format("  键槽 b={0}  t2={1}  槽顶尺寸 d+t2={2:0.0}", ky[0], ky[1], p.Bore + ky[1]));
    ...
    static void Main()
    {
        ...
        a.Bw = 15; a.Bore = 15; a.Keyway = true; a.Npts = 24;
        Show("A  细齿 + 键槽", a);

        GearParams b = new GearParams();
        b.Mn = 2; b.Z = 12; b.X = 0; b.Bw = 20; b.Bore = 8; b.Keyway = true; b.Npts = 24;
        Show("B  根切演示 (x=0)", b);

        GearParams c = new GearParams();
        c.Mn = 2; c.Z = 12; c.X = 0.5; c.Bw = 20; c.Bore = 8; c.Keyway = true; c.Npts = 24;
        Show("C  变位修正 (x=0.5) —— 与 B 对比", c);

        GearParams d = new GearParams();
        d.Mn = 2; d.Z = 40; d.X = 0; d.Bw = 20; d.Bore = 20; d.Keyway = true; d.Npts = 24;
        Show("D  与默认 z=20 配对的大齿轮", d);

        GearParams e = new GearParams();
        e.Mn = 3; e.Z = 18; e.AlfN = 20; e.Beta = 15; e.X = 0.2;
        e.Bw = 30; e.Bore = 25; e.Keyway = true; e.Npts = 24;
        Show("E  斜齿 + 变位 (重载)", e);
    }
}
